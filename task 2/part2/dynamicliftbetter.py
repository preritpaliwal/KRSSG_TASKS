...
def operate(l,pas,q,inlift=[]):
    ...
    while passengers[pas[0]].start!=l.currentfloor:
        ...
        for i in wait:
            if passengers[i].start==l.currentfloor:
                if l.direction==np.sign(passengers[i].end-l.currentfloor):
                    if abs(passengers[pas[0]].start-l.currentfloor)>= abs(passengers[i].end-l.currentfloor):
                        l.fsm.transition("torest")
                        if q==2:
                            print("                                                       ",end=" ")
                        print(f"HI passenger {i}")
                        inlift.append(i)
                        wait.remove(i)

        for e in inlift:
            if passengers[e].end == l.currentfloor:
                l.fsm.transition("torest")
                if q==2:
                        print("                                                       ",end=" ")
    ...
    for i in pas:
        if passengers[i].start==l.currentfloor:
            if np.sign(passengers[i].end-l.currentfloor)==np.sign(passengers[pas[0]].end-l.currentfloor) and passengers[i].end-l.currentfloor<=passengers[pas[0]].end-l.currentfloor:
                l.fsm.transition("torest")
                l.fsm.execute()
                if q==2:
                    print("                                                       ",end=" ")
    ...
    while passengers[pas[0]].end!=l.currentfloor:
        ...
        for i in wait:
            if passengers[i].start==l.currentfloor:
                if l.direction==np.sign(passengers[i].end-l.currentfloor):
                    if abs(passengers[pas[0]].end-l.currentfloor)>= abs(passengers[i].end-l.currentfloor):
                        l.fsm.transition("torest")
                        if q==2:
                            print("                                                       ",end=" ")
                        print(f"HI passenger {i}")
