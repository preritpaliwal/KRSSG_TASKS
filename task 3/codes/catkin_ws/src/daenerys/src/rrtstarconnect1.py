...
class RRTGraph:
    ...
    def show_node(self,x,y,color = magenta):
        cv2.circle(self.img,(x,y),2,color,-1)
        # if comp(color,grey):
...
def rrtstarconnect():
    ...
    start2 = (int(97*300/382),int(29*300/337))
    start3 = (int(29*300/382),int(257*300/337))
    goal2 = (int(204*300/382),int(312*300/337))
    goal1 = (int(367*300/382),int(107*300/337))
    ...
    map = RRTMap(start3,goal1,(img.shape[0],img.shape[1]),img)
    graph1 = RRTGraph(start3,goal1,(img.shape[0],img.shape[1]),img)
    graph2 = RRTGraph(goal1,start3,(img.shape[0],img.shape[1]),img)
    iterations = 0
    k=0
    global p4
        ...
        if f1 == 1:
            ...
            if inear2 != -1:
                ...
                if f1 == 1:    
                    ...
                    graph1.show_node(graph2.x[inear2],graph2.y[inear2],grey)
                    # show(img)
                    cv2.line(img,(x1[-1],y1[-1]),(x1[parent1[-1]],y1[parent1[-1]]),blue,1)
        ...
        if f2 == 1:
            ...
            if inear1 != -1:
                ...
                if f2 == 1:    
                    ...
                    graph1.show_node(graph1.x[inear1],graph1.y[inear1],grey)                                  
                    cv2.line(img,(x2[-1],y2[-1]),(x2[parent2[-1]],y2[parent2[-1]]),blue,1)
